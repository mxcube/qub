...
inline void _i420_2_image(const unsigned char* data,
			 ...
			 bool scalingFlag)
{
#define _I420_YVU_SCALING_BRGA(yPt,imagePt)		\
  y = int(9535 * ((A * *yPt + B) - 16));		\
  red = (y + redChro) >> 13;				\
  if(red > 255) red = 255;				\
							\
  green = (y + greenChro) >> 13;			\
  if(green  > 255) green = 255;				\
							\
  blue = (y + blueChro) >> 13;				\
  if(blue > 255) blue = 255;				\
...
#define _I420_YVU_BRGA(yPt,imagePt)			\
  y = int(9535 * (*yPt - 16));				\
  red = (y + redChro) >> 13;				\
  if(red > 255) red = 255;				\
							\
  ...
  *imagePt = 0xff000000 | (red << 16) | (green << 8) | blue; \
  ...
  const unsigned char *Yline2 = data + row;
  const unsigned char *V = data + aNbPixel;
  const unsigned char *U = V + (aNbPixel >> 1);
  unsigned int *anImageLine2Pt = anImagePt + aNbPixel;
  if(scalingFlag)
    {
      float A, B;
      if(maxValue - minValue)
      ...
      else
	{
      ...
      for(;aNbPixel;aNbPixel -= 4,++V,++U)
	{
	  int redChro = (*V - 128) * 13074;
	  int greenChro = -6660 * (*V - 128) - 3203 * (*U - 128);
	  int blueChro = 16531 * (*U - 128);
	  int y,red,green,blue;
	  _I420_YVU_SCALING_BRGA(Yline1,anImagePt);
	  _I420_YVU_SCALING_BRGA(Yline1,anImagePt);
	  _I420_YVU_SCALING_BRGA(Yline2,anImageLine2Pt);
	  _I420_YVU_SCALING_BRGA(Yline2,anImageLine2Pt);
	}
    }
  else
      ...
      for(;aNbPixel;aNbPixel -= 4,++V,++U)
	{
	  ...
	  int blueChro = 16531 * (*U - 128);
	  int y,red,green,blue;
	  _I420_YVU_BRGA(Yline1,anImagePt);
	  _I420_YVU_BRGA(Yline1,anImagePt);
	  _I420_YVU_BRGA(Yline2,anImageLine2Pt);
	  _I420_YVU_BRGA(Yline2,anImageLine2Pt);
	}
    }
}
