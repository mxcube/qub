...
static PyObject* down_size(PyObject *self, PyObject *args)
{
  ...
  double *aDest = (double*)calloc(sizeof(double),(aDestWidth + 1) * (aDestHeight + 1)); // and a column and a line to avoid limit test
...
*/
    ...
    for (i=0;i<npoints;i++){
	    ...
	    std::cout << yresult << std::endl; // TO BE REMOVED
          }
        }
